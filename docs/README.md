...
| [Report enrichment contract](developer/report_enrichment_contract.md) | Implemented | Process/card scope, evidence states, bounded selection, hit grouping, and report compatibility invariants |
| [Thread evidence contract](developer/thread_evidence_contract.md) | Implemented | Bounded thread-stream parsing, recorded starts versus captured IPs, context conflicts, report anchoring, and shared hunter qualifications |
| [PE image profile contract](developer/pe_image_profile_contract.md) | Frozen contract; partially implemented | Canonical in-memory PE profile: image identity, actual-versus-preferred base addressing, the sixteen directory descriptors, component coverage states, cache identity, and projection ownership. The internal memory-sourced collector and the correlation layer implement the raw-profile, staged-acquisition, and §8 consistency/correlation observation subset and reach no shipped path; cache reuse, projections, and disk-reference collection remain future work |
