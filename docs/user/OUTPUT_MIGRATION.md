...
|---|---|---|
| `--list`, `--modules`, `--threads`, `--process`, `--sysinfo`, `--handles`, `--profile`, `--diff`, `--extract`, `--strings`, `--report`, `--hunt` | v2.16 (current) | [`dumpex-output-v2.16.schema.json`](../../dumpex/schemas/dumpex-output-v2.16.schema.json) |
...
## Producer behavior within v2.16

Not every change to what dumpex emits changes the wire shape. These narrow what
a v2.16 document contains without changing what a v2.16 document may contain, so
