...
Layout:
  records.py    canonical record dataclasses (ModuleRecord, ThreadRecord,
                MemoryRegionRecord, ProcessInfoRecord, Diagnostic) -- plain
                dataclasses with an explicit to_dict(), the same house
                style as dumpex.hunt._finding.Finding.
  ...
  serializer.py to_json(): a STRICT encoder that raises on any type it
                ...
                str(obj) fallback.
  csv_export.py records -> CSV rows (trivial: every record is already a
                flat dict via to_dict()).
