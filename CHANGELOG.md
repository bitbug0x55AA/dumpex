...
- `--report` and `--extract` no longer treat a bare `MZ` prefix or a benign
  read-only mapped PE as an injected PE. Incomplete PE validation is reported
  as partial coverage, and extraction diagnostics explain the evidence gap.
- Memory classification now distinguishes private, mapped, and image memory
  from unavailable type or registration evidence. `--report-string` summaries
  expose the same distinctions; `hollowing` reports the observed memory type,
  and YARA's `PE_In_Private_Memory` requires confirmed private memory.
- `--threads` and `--report` distinguish recorded thread start addresses from
  captured instruction pointers, including missing, zero, and disputed context.
  Reports explain which address places each thread in the examined region and
  make clear when the thread's current location was not examined.
- `--report-tid` now finds threads captured only in the base thread stream and
  examines their captured instruction pointer when no usable start address is
  available. This can reveal findings or incomplete coverage in a region that
  previously went unexamined.
- Unavailable or invalid thread start addresses no longer produce false
  unbacked-thread findings or pipe proximity leads. Truncated thread records
  retain captured facts instead of fabricating zero values or disappearing
  from `--diff`; combined thread flags are reported correctly.
- Missing thread records, incomplete thread streams, and unestablished start
  addresses now produce explicit coverage limitations where required, rather
  than a complete or clean result over evidence that could not be examined.
- Injection, stomping, and pipe findings now qualify disputed or unverifiable
  thread context consistently, with corrected thread counts in the caveats.
  These qualifications do not change hunter scores or confidence.

### Changed

- Published output schema v2.20 for memory classification, PE validation, and
  thread address/context evidence. See
  [Output Schema Migration](docs/user/OUTPUT_MIGRATION.md) for compatibility
  details; earlier documents continue to validate against their own schema.
- Required PE or thread evidence gaps can now change coverage from `complete`
  to `partial` and the exit code from 0 to 3. Injection hunts with unexamined
  thread starts can report `INCONCLUSIVE` instead of `CLEAN`.
...
- Prevented duplicate YARA suppression output.
...
- Current commands emit schema v2.20.
