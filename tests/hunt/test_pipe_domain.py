"""Contract tests for the canonical named-pipe domain model
(dumpex.hunt._domain.CheckResult, dumpex.hunt.pipe.domain's
CoverageSnapshot/PipeEvidence/PipeReport, and the dumpex.hunt.pipe.models
Evidence value objects they are built from). Mirrors
tests/hunt/test_stomping_domain.py, tests/hunt/test_encoding_domain.py and
...
acceptance criteria this module applies to the pipe hunter:
  ...
  3. The model retains no minidump, resolver, projector, verbosity,
     Finding, HunterRecord, dict, CoverageTracker, ScanBudget, or
     console-string reference -- poison objects of each kind are rejected
     at construction.
...
@pytest.mark.parametrize("not_a_sequence", [
    pytest.param({"handle_pipes": ()}, id="dict"),
    pytest.param("Handle=0x88", id="bare-string"),
    pytest.param(iter(()), id="generator"),
    pytest.param({_handle()}, id="set"),
])
