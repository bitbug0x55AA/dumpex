...
| [PE image profile contract](developer/pe_image_profile_contract.md) | Frozen contract; partially implemented | Canonical in-memory PE profile: image identity, actual-versus-preferred base addressing, the sixteen directory descriptors, component coverage states, cache identity, and projection ownership. The internal memory-sourced collector implements the raw-profile and staged-acquisition subset and reaches no shipped path; cache reuse, consistency observations, projections, and disk-reference collection remain future work |
